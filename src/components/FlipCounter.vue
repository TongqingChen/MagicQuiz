<!-- 翻页效果 倒计时组件 -->
<template>
    <div :class="['vue-countdown-component', { theme2: split }, { ie: isIE }]">
        <template v-for="(item, index) in timeArray" :key="index">
            <div :class="['time-box']">
                ...
                <!-- 翻页动画div -->
                <div :class="['face', { anime: isAnimate[index] }]" @animationend="onAnimateEnd(index)">{{ timeArrayT[index]
                }}</div>
...
export default {
    ...
    props: {
        seconds: { type: Number, default: 10 }, // 截止时间
        ...
        timeArray(newV, oldV) {
            ...
            newV.forEach((value, index) => {
                if (value !== oldV[index]) {
                    ...
                    this.isAnimate[index] = true;
                }
            });
            setTimeout(() => {
                diff.forEach(item => {
                    this.timeArrayT[item.index] = item.value;
                });
            }, 350);
        },
        endTime(newV) {
    ...
    methods: {
        ...
        start(step = 1000) {
            ...
            this.timer = setTimeout(() => {
                ...
                if (!this.split) {// 不分开
                    ...
                    arr.push(String(sec).padStart(2, "0"));
                } else { // 分开
                    type >= 4 && (arr = String(day).padStart(2, "0").split(""));
                    type >= 3 && (arr = arr.concat(String(hour).padStart(2, "0").split("")));
                    type >= 2 && (arr = arr.concat(String(min).padStart(2, "0").split("")));
                    arr = arr.concat(String(sec).padStart(2, "0").split(""));
                }
                this.timeArray = arr;
                if (this.stop) {
                    return
        ...
        onAnimateEnd(index) {
            this.isAnimate[index] = false;
        },
        isTimeUnitShow(index) {
            return this.arr.includes(index) && (index != this.timeArray.length - 1 || this.timeUnit[3]);
        },
        setTimeUnit(index) {
            switch (index) {
