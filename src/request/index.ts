...
export class Api {
    ...
    //sub_name为空Null--表示全部
    static getWrongSetsMixedBySubInfos(sub_infos: string[] | null = null) {
        const user_id = Api.loadUserIdFromStorage();
        return Axios({
            url:
                'wrongsets_mixpost/?user_id=' +
                (sub_infos
                    ? `${user_id}&sub=${sub_infos[0]}&grd=${sub_infos[1]}&vol=${sub_infos[2]}`
                    : `${user_id}`),
            method: 'GET',
        });
    }

    static getFavouriteSetsBySubInfos(sub_infos: string[] | null = null) {
        const user_id = Api.loadUserIdFromStorage();
        return Axios({
            url:
                'favourite_set/?user_id=' +
                (sub_infos
                    ? `${user_id}&sub=${sub_infos[0]}&grd=${sub_infos[1]}&vol=${sub_infos[2]}`
                    : `${user_id}`),
            method: 'GET',
