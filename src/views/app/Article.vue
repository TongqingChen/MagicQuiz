<template>
    <div class="common-layout">
        <el-container>
            ...
            <el-main>
                <h1
                    v-if="currArticle.id > 0"
                    align="center"
                    style="margin: 10px; font-size: larger"
                >
                    {{ currArticle.title }}
                </h1>
                ...
                <div
                    v-if="
                        currArticle.attachment &&
                        currArticle.attachment.length > 0
                    "
                >
                    <VueOfficeDocx
                        v-if="currArticle.attach_type == 0"
                        :src="currArticle.attachment"
                        @rerender="() => {}"
                        @error="() => {}"
                    />
                    <VueOfficePdf
                        v-if="currArticle.attach_type == 1"
                        :src="currArticle.attachment"
                        :rerender="() => {}"
                        @error="() => {}"
                    />
...
onMounted(() => {
    ...
    Api.getArticles().then((res) => {
        res.data.results.forEach((e: any) => {
            var cid = meta.findIndex((a) => {
                return a.category == e.category;
            });
            if (cid < 0) {
