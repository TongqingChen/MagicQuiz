<template>
    <el-container class="layout-container-exam" style="height: 100%">
        <el-header>
            ...
            <FlipCounter
                :seconds="examInfo.exam_seconds"
                :type="3"
                :split="blink"
                :timeUnit="[':', ':', ':']"
                :stop="examInfo.state == ExamState.FINISHED"
                @timeUp="uploadExamResults"
            />
            <el-button
                link
                type="primary"
                :disabled="examInfo.state != ExamState.ONGOING"
                @click="submitQuiz"
                >交卷</el-button
            >
        </el-header>
        <el-container>
            <el-aside width="164px">
                <el-scrollbar style="height: 100%">
                    <div v-for="qs in examInfo.meta" style="margin: 5px">
                        <el-divider v-if="qs.qList.length > 0">{{
                            qs.typeName
                        }}</el-divider>
            ...
            <el-main v-if="activeQ.index >= 0">
                <div style="padding-left: 6px; padding-top: 5px">
                    ...
                    <div
                        class="answer"
                        v-if="examInfo.state == ExamState.FINISHED"
                    >
                    ...
                    <el-button-group style="padding-left: 6px">
                        <el-button
                            :disabled="
                                activeQ.index < 0 ||
                                activeQ.index >= ijPairs.length ||
                                examInfo.state != ExamState.ONGOING
                            "
                            type="primary"
                            size="small"
                            @click="onDoubtBtnClicked"
                            >❓存疑</el-button
                        >
                        <el-button
                            :disabled="activeQ.index == 0"
                            type="primary"
                            :icon="ArrowLeft"
                            size="small"
                            @click="onQuestionClicked(activeQ.index - 1)"
                            >上题</el-button
                        >
                        <el-button
                            :disabled="activeQ.index == ijPairs.length - 1"
                            type="primary"
                            size="small"
                            @click="onQuestionClicked(activeQ.index + 1)"
                            >下题<el-icon class="el-icon--right">
                                <ArrowRight /> </el-icon
                        ></el-button>
                    </el-button-group>
                    <el-button
                        v-if="examInfo.state == ExamState.FINISHED"
                        type="success"
                        :icon="activeQ.is_favourited ? StarFilled : Star"
                        round
                        size="small"
                        @click="onFavouriteClicked"
                        :disabled="activeQ.is_favourited"
                        >{{
                            activeQ.is_favourited ? '已收藏' : '收藏'
                        }}</el-button
                    >
                    <div v-if="examInfo.state == ExamState.ONGOING">
                        <el-radio-group
                            v-model="activeQ.userAnswer"
                            v-if="activeQ.type == QueType.CHOICE"
                        ...
                        <el-radio-group
                            v-model="activeQ.userAnswer"
                            v-if="activeQ.type == QueType.LOGIC"
                            @change="onAnswerSelected"
                        >
                        ...
                        <el-input
                            v-model="activeQ.userAnswer"
                            placeholder="请填入答案，多个答案以英文分号隔开，如: 134;2;abc"
                            v-if="activeQ.type == QueType.BLANK"
                            clearable
                            @blur="onAnswerSelected"
                        >
                        </el-input>
                    </div>
                </div>
                <div class="question">
                    <div class="title">
                        {{ activeQ.index + 1 }}. ({{ activeQ.score }}分){{
                            activeQ.title
                        }}
                    </div>
                    <el-image
...
const getQuestionList = async () => {
    ...
    examInfo.subjectName = String(route.query.subject);
    examInfo.gradeName = String(route.query.grade);
    examInfo.volumeName = String(route.query.volume);

    examInfo.exam_seconds = Number(route.query.exam_seconds);
    examInfo.question_num = 0;

    await Api.getQuestionTypes()
        .then((res) => {
            qTypes = res.data;
        })
        .catch((err) => {
    ...
    if (examInfo.id > 0) {
        await Api.getQuestionListByQuizId(examInfo.id).then((res) => {
            ...
            questions.forEach((q) => {
                ...
                _q.index = i++;
                _q.userAnswer = '';
                _q.displayType = 'default';
                examInfo.meta[t_id].qList.push(_q);
                examInfo.scores += q.score;
            });
            examInfo.question_num = questions.length;
            examInfo.state = ExamState.ONGOING;
        });
    } else if (examInfo.id == -1) {
        //-1表示错题集
        await Api.getWrongSetsMixedBySubInfos([
            ...
            wrong_set.forEach((w) => {
                ...
                examInfo.meta[t_id].qList.push({
                    ...
                    userAnswer: '未作答',
                    score: w.score,
                    is_favourited: w.is_favourited,
                    wrong_times: w.wrong_times,
                    copyFrom: () => {},
                });
        ...
        await Api.getQuestionListRandom(
            [examInfo.subjectName, examInfo.gradeName, examInfo.volumeName],
            [choice_num, logic_num, blank_num, coding_num]
        ).then((res) => {
            let questions: Question[] = res.data;
            questions.sort((q1, q2) => {
                return q1.type - q2.type;
    ...
    } else if (examInfo.id == -3) {
        // -3 表示收藏题集
        await Api.getFavouriteSetsBySubInfos([
            examInfo.subjectName,
            examInfo.gradeName,
            examInfo.volumeName,
        ]).then((res) => {
...
const uploadExamResults = async () => {
        ...
        qs.qList.forEach((q) => {
            ...
            if (q.type != QueType.CODING) {
                ...
                results.questions.push({
                    ...
                    is_correct: q.userAnswer == q.answer,
                });
            }
        });
    ...
    qTypes.forEach((q) => {
        ...
        if (correct_count[i] + error_count[i] > 0) {
            rsl_str += `【${q[1]}】`;
            correct_count[i] > 0 &&
                (rsl_str += `<b>${correct_count[i]}</b> ✅, `);
            error_count[i] > 0 &&
                (rsl_str += `<b>${error_count[i]}</b> ❌<br/>`);
        }
        i++;
    });
    results.meta.user = user_id;
    results.meta.quiz = examInfo.id;
    results.meta.rel_score = Math.round(
        (results.meta.abs_score * 100) / total_score
    );
    results.meta.use_minutes = Math.max(
        1,
        Math.round((Date.now() - examInfo.start_time) / 1000 / 60)
    );
    await Api.postQuestionsResult(results.questions);
    if (examInfo.id >= 0) {
        //错题集、题库随机考试、收藏集不提交考试记录，仅提交错题记录
...
const onFavouriteClicked = async () => {
    await Api.postFavouriteQuestion(activeQ.id).then(() => {
        var i = ijPairs[activeQ.index][0];
        var j = ijPairs[activeQ.index][1];
        examInfo.meta[i].qList[j].is_favourited = true;
        activeQ.is_favourited = true;
